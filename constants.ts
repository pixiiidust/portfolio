...
export const BIO_DATA = {
  headline: "OPERATOR BIO",
  paragraphs: [
    "From product strategy down to the details, I love every part of building products people actually enjoy using.",
  ...
  closingParagraph:
    "Right now that means AI-native applications, where the challenge is delivering a tasteful experience without shipping slop. The bar is taste, and the test is whether people keep coming back.",
};
...
export const CASE_STUDIES: CaseStudy[] = [
  {
    ...
    links: [],
    insight:
      "The bottleneck was never engineering speed. It was feedback latency. Once you can watch real users struggle in hours instead of weeks, you stop shipping features that were going to fail anyway.",
    context:
      "We were building an AI text-summary app with a map-like stretching UI for scanning long-form content. The teams were stuck on a monthly release cycle, because interpreting user feedback was slow. We only learned whether a feature had product-market fit once every 30 days, which put the startup's runway at risk.",
    goal: "Compress the idea-to-validation loop from 4 weeks to under 12 hours, so we learn faster.",
    hypothesis:
      "If we replace slow, manual work with a high-velocity loop built on agentic AI (Claude and Hermes agent harnesses for prototyping, with subagent orchestration to synthesise feedback), we can validate a feature concept in a single day with real users and stop wasting engineering on low-value features.",
    problem: [
    ...
    solution:
      "I built a 12-hour validation pipeline to watch real customers struggle through the product before we committed code.\n\n" +
    ...
    reflection:
      'The tools were not the point. The point was making a "kill" decision cheap and same-day. Killing four features early was worth more than shipping them.',
  },
  {
    ...
    links: [],
    insight:
      "Downloads are a vanity metric. The habit forms at the first booking, so I optimized for Time to First Booking and moved the highest-value regulars over first.",
    context:
      "A beauty chain with 12 outlets and over 100k customers ran on a legacy phone-booking system. The manual process capped growth, hid behavioural data, and cost a lot to staff.",
    goal: "Move customers from phone to a mobile app, without alienating a non-technical base.",
    hypothesis:
      "If we cut booking friction by moving from synchronous calls to an asynchronous app, and optimize for Time to First Booking, we create a habit-forming loop that retains better than the phone channel.",
    problem: [
  ...
  },
  {
    ...
    links: [],
    insight:
      'Low sales on the large sizes were not low demand. They were unmet demand. The UI blocked the add-to-cart, so "zero interest" was a false negative the interface created.',
    context:
      "A client was six months into a loss streak. The firm stocked cash-cow small sizes by historical volume, and larger sizes were listed but usually out of stock. Analytics showed low sales and low LTV on the large sizes, so stakeholders concluded there was no demand.",
    goal: "Find the real cause of the revenue bleed, and prove that low sales did not mean low demand.",
    hypothesis:
      "Conceived through a Return on Customer lens. If we capture user intent at the size-selection step, specifically for out-of-stock variants, we can show the low LTV on large sizes is actually unmet demand, which justifies a change in inventory strategy.",
    problem: [
    ...
    solution:
      '- MVP: we replaced the out-of-stock dead-end with an "Availability Request." Instead of a greyed-out button, users saw "Notify me when available."\n' +
...
export const PROJECTS: Project[] = [
  {
    ...
    insight:
      'A reasoning style can be defined and measured. I turned "reason like him" into six observable moves (reframe the question, argue from first principles, use a concrete analogy, weigh interests over sentiment, bound uncertainty, be direct), so a fuzzy goal became a scored eval.',
    problem:
      "Small open-source models are cheap and private, but can one take on a domain expert's way of reasoning, not just their facts? Lee Kuan Yew was the test case, chosen for a distinctive method and a large public record.",
    approach:
      "Scraped public transcripts, built a two-part dataset (interview exchanges, plus speeches paired with generated questions), and LoRA fine-tuned Qwen3-14B on one consumer GPU. Judged blind against a held-out set of interviews.",
    result:
      "The tuned model beat the untuned base on nearly every move, with the voice score rising from 2.04 to 2.88.",
    reflection:
      "Two calls mattered more than the training. I shipped the less-overfit checkpoint over the one with the better training loss, because it generalised better on unseen questions. And I published the dataset as a reproducible recipe with pointers to the source, rather than redistributing the archive itself.",
  },
  {
    slug: "pixi-wiki",
    kind: "project",
    ...
    links: [{ label: "LAUNCH", href: "https://pixiiidust.github.io/pixi-wiki/" }],
    insight:
      "Publish the content as structured, agent-readable Markdown with an index, and agents retrieve exactly what they need instead of scraping HTML.",
    problem:
      "My notes and docs needed to serve two audiences at once. People browsing, and AI agents that read and cite them. Agents normally scrape a website, which is slow, noisy, and token-heavy.",
    approach:
      "One Markdown corpus, published as a clean human wiki, and exposed to agents through llms.txt, a read-only MCP server (list, search, read), and an index.json registry.",
    result:
      "Agents retrieve source-backed answers from the same maintained pages people browse, instead of scraping raw dumps, which cuts hallucinations and token waste.",
  },
  {
    slug: "j-space-replay",
    kind: "project",
    ...
    insight:
      "An interpretability lens can surface a model's internal state frame by frame, including reasoning that never reaches its answer.",
    problem:
      'You can read a vision-language model\'s answer, but not what it was reasoning about internally, including what it "thinks" but never says.',
    approach:
      "Qwen2.5-VL with logit-lens and Jacobian (J-lens) decoding, and per-layer residual capture, replayed over preset video clips in the browser, or run on your own clips with a local pipeline.",
    result:
      'Visitors browse a layer-by-layer grid of the top word the model "read" for each output token, and can spot reasoning it computed but never said. Explore preset clips instantly, or upload your own.',
  },
  {
    ...
    links: [{ label: "LAUNCH", href: "https://pixiiidust.github.io/planned-program-intel/" }],
    insight:
      "The best version is not a chatbot. It is an inbox of decisions, where each one routes to the right owner with its justification, and every human choice becomes evidence the next decision cites.",
    problem:
      "In AI-native tools, decisions pile up on users as a queue of alerts to chase and triage, and the reasoning behind past calls is lost.",
    approach:
    ...
    result:
      "A 60-second demo that shows the pattern. Decisions route to the right owner with their justification attached, and every human resolution becomes institutional memory the next decision cites. No autonomous execution.",
  },
];
